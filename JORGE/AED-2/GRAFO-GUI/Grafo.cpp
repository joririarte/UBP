...
void Grafo::SetPonderado(bool p)
{
	ponderado = p;
}
...
void Grafo::importarMatrizAdyacencia(int** m, int tam)
{
	//creo los vertices con etiquetas de letras
	string origen = "A";
	string destino = "A";
	for (int i = 0; i < tam; i++) {
		origen[0] += i;
		this->InsertaVertice(origen);
		origen = "A";
	}
	origen = "A";
	//Creo las aristas
	for (int i = 0; i < tam; i++) {
		...
		for (int j = 0; j < tam; j++) {
			if (m[i][j] != 0) {
				destino[0] += j;
				this->InsertaArista(origen, destino, m[i][j]);
				destino = "A";
			}
		}
		origen = "A";
...
bool** Grafo::AristasResaltadas()
{
	bool** resaltadas = new bool* [tamano];
	for (int i = 0; i < tamano; i++) {
		resaltadas[i] = new bool[tamano];
		for (int j = 0; j < tamano; j++)
			resaltadas[i][j] = false;
	}
	cout << endl << endl;
	Vertice* vi = primero;
	while (vi!=NULL)
	{
		...
		while (aj!=NULL)
		{
			int i = vi->nombre[0] - 65;
			int j = aj->dest->nombre[0] - 65;
			resaltadas[i][j] = aj->resaltada;
			aj = aj->sig;
		}
		vi = vi->sig;
	}
...
void Grafo::PrimeroEnAnchura(string origen, string destino)
{
	...
	if (vori != NULL && vdest != NULL)
	{
		...
		while (!cola.empty())
		{
			...
			//Si no está en la lista de visitados..
...
void Grafo::MostrarRutaEncontrada(stack<pair<Vertice*, Vertice*>> pilaPar, Vertice* vdest)
{
	...
	while (!pilaPar.empty())
	{
		...
		if (!pilaPar.empty()) {
			...
			Arista* ari = destinoActual->ari;
			while (ari != NULL)
			{
				if (ari->dest == pilaPar.top().second)
					ari->resaltada = true;
				ari = ari->sig;
			}
		}
...
void Grafo::Prim(string origen)
{
	...
	while (vi != NULL)
	{
		...
		Arista* aj = vi->ari;
		//llena la fila con los costos de la arista
		while (aj != NULL)
		{
			matrizady[vi][aj->dest] = aj->precio;
			aj = aj->sig;
