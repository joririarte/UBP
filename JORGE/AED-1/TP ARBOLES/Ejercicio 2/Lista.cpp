...
void Lista::insertarAntes(int data, int op)
{
	...
	//y el nuevo nodo apuntará a donde apuntaba raiz
		...
		raiz = new Nodo(data, op, raiz);
}

//Inserta un dato en medio de la lista
void Lista::insertarEntre(int data, int anterior, int op)
{
	...
	if (raiz != nullptr)
		...
		buscar(anterior)->sig = new Nodo(data, op, buscar(anterior)->sig);
}
...
void Lista::mostrar()
{
	...
	if (raiz != nullptr) {
		...
		while (recorrido != nullptr) {
			std::cout << "Clave: " << recorrido->info 
					  << " Operacion: "<<recorrido->operacion
					  << std::endl;
...
Lista::Nodo* Lista::getNext(Nodo* n)
{
	return n->sig;
}
...
//busca un dato por clave y operacion
Lista::Nodo* Lista::buscar(int dato, int operacion)
{
	Nodo* rec = raiz;
	while (rec != nullptr) {
		if (rec->info == dato && rec->operacion == operacion)
			return rec;
		rec = rec->sig;
	}
	return nullptr;
...
Lista::Nodo::Nodo(int data, int op)
{
	info = data;
	operacion = op;
	sig = nullptr;
...
Lista::Nodo::Nodo(int data, int op, Nodo* n)
{
	info = data;
	operacion = op;
	sig = n;
